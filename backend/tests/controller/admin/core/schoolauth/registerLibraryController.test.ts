import request from 'supertest';
import express from 'express';
import routes from '../../../../../src/modules/admin/routes/core/schoolauthroutes/libraryRoutes';
import { prisma } from '../../../../../src/db/prisma';

jest.mock('../../../../../src/db/prisma', () => {
  const user = { findMany: jest.fn() };
  const library = { findUnique: jest.fn() };
  return { prisma: { user, library } };
});

const app = express();
app.use(express.json());
app.use('/', routes);

beforeAll(() => {});
afterAll(() => { jest.resetAllMocks(); });

describe('Library routes', () => {
  beforeEach(() => jest.clearAllMocks());

  it('lists libraries', async () => {
    (prisma.user.findMany as jest.Mock).mockResolvedValue([{ id: 'l1' }]);
    const res = await request(app).get('/library');
    expect(res.status).toBe(200);
    expect(res.body).toEqual([{ id: 'l1' }]);
  });

  it('handles error', async () => {
    (prisma.user.findMany as jest.Mock).mockRejectedValue(new Error('fail'));
    const res = await request(app).get('/library');
    expect(res.status).toBe(500);
  });
});
