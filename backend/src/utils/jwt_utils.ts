...
import { compareUserPermissionHandler } from "./permission-utils";
...
export const authenticateToken = async (
  ...
  try {
    ...
    const userObj = await prisma.user.findUnique({ where: { id: decoded.userId } });
    ...
    return new Promise((resolve, reject) => {
      jwt.verify(
        ...
        async (err: any, user: any) => {
          // if (err) {
          //   return res.status(403).json({ status: "error", errors: err.message });
          // }
  ...
  return (request: Request, response: Response, next: NextFunction) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (user) {
        ...
        if (!userObj) {
          return response.status(403).json({ status: "error", errors: "Invalid access" });
        }
