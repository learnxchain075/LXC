...
const ProjectDashboard = () => {
  ...
  const issueMap = ISSUE_OPTIONS.reduce(
    (acc, cur) => {
  ...
  const handleUpdateProject = async (p: Project) => {
    ...
    const description =
      prompt("Project description", p.description || "") || "";
  ...
  const handleAddRepo = async (projectId: string) => {
    const repoName = prompt("GitHub repo (owner/repo)");
    if (!repoName) return;
    const token = prompt("GitHub token");
    if (!token) return;
    await addGitHubRepo(projectId, { repoName, token });
    fetchProjects();
  };
        ...
        {projects.map((p) => (
          <div key={p.id} className="mb-4 border rounded p-3">
            ...
            {p.githubRepos && p.githubRepos[0] && (
              <p className="mb-2">
                GitHub:{" "}
                <a href={p.githubRepos[0].repoUrl} target="_blank">
                  {p.githubRepos[0].repoUrl}
                </a>
              </p>
            )}
            <button
            ...
            <TaskModal
              show={showTaskModal && activeProjectId === p.id}
                        ...
                        .map((t) => (
                          <li key={t.id} className="mb-2 p-2 border rounded">
                            <div className="d-flex justify-content-between">
                              <span
                                className={`text-${issueMap[t.issueType as IssueType]?.color || "secondary"} me-2`}
                              >
