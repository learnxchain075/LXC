...
import { createSubject } from "../services/teacher/subjectServices";
...
//   // var teacherIdsend:string="";
//   const [teacherIdsend, setteacherIdSend] = useState<any>({
//     teacherId: "",
//   });
...
//  });
  ...
  useEffect(()=>{
    const fetchData = async () => {
      try{
        const response = await getTeacherByschoolId(localStorage.getItem("schoolId") as string);
        // console.log("claas name ",response.data);
  ...
  // const data = teacherSubject;
  const dropdownMenuRef = useRef<HTMLDivElement | null>(null);
  const handleApplyClick = () => {
    if (dropdownMenuRef.current) {
      dropdownMenuRef.current.teacherList.remove("show");
    }
  };
  const columns = [
    {
...

    {
      ...
      render: (text: string, record: any, index: number) => (
        <>
          <Link to={routes.classTimetable} state={{ id: record.id }} className="link-primary">
            {record.user.name}
          </Link>
        </>
      ),
          ...
          <div className="content">
            ...
            <div className="card">
              ...
              <div className="card-body p-0 py-3">
                {/* Guardians List */}
                <Table columns={columns} dataSource={teacherList ?? []} Selection={true} />
