...
import { getRoute, getRoutesBySchoolId } from "../../../../services/transport/busRouteServices";
...
const TransportRoutes = () => {
  ...
  const fetchdata = async () => {
    try {
    const response = await getRoutesBySchoolId(localStorage.getItem("schoolId") || "");
    setTransportRouteList(response?.data as any);
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "busStops",
      render: (busStops: any[]) =>
        busStops?.length ? busStops.map((stop) => stop.name).join(", ") : "None",
    },
    {
      title: "Bus Number",
      dataIndex: "bus",
      render: (bus: any) => bus?.busNumber || "N/A",
    },
    {
        title: "Status",
        dataIndex: "status",
    ...
    Add Route
          ...
          <div className="card">
            ...
            <div className="card-body p-0 py-3">
              {/* Student List */}
              <Table dataSource={transportRouteList} columns={columns} 
              />
              {/* /Student List */}
            </div>
          </div>
