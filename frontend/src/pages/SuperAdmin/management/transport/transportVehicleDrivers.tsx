...
import { getDriverbyid } from "../../../../services/transport/driverServices";
import { Table } from "antd";
import { IBusResponse } from "../../../../services/types/transport/busService";
...
const TransportVehicleDrivers = () => {
  ...
  const [busList, setBusList] = useState<IBusResponse[]>([]);
  const fetchdata = async () => {
    try {
      const response = await getDriverbyid(localStorage.getItem("schoolId") || "");
  ...
  const fetchBuses = async () => {
    try {
      const response = await getBusbyID(localStorage.getItem("schoolId") || "");
      setBusList(Array.isArray(response?.data) ? response.data as IBusResponse[] : []);
    } catch (error) {
  ...
  const columns = [
    ...
    // },
    {
      ...
      render: (busId: string) => {
        ...
        return <span>{bus ? bus.busNumber : "N/A"}</span>;
      },
    },
